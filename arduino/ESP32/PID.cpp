...
void PID::setEEPROM()
{
  ...
  do
  {
    ...
    num_in = strs[2].toInt();
    num_out = strs[3].toInt();
  } while (len != num_in + num_out + 3 + 1);
  float* input_coeff = new float[num_in];
  float* output_coeff = new float[num_out];
  for (int i = 4; i - 4 < num_in; i++)
  {
    input_coeff[i - 4] = strs[i].toFloat();
  }
  for (int i = 4 + num_in; i - 4 - num_in < num_out; i++)
  {
    input_coeff[i - 4 - num_in] = strs[i].toFloat();
  }
